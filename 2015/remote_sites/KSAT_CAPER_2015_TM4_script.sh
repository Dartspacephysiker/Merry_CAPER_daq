...
RANGE=1111 # 2015/11/10 David McGaw verified with me that this is the correct range
...
if [ ${NCHAN} -gt 0 ] && [ ${NCHAN} -le 4 ] && comedi_config --read-buffer 65536 /dev/comedi0 && comedi_test -s0 /dev/comedi0 >/dev/null 2>&1 ; then 
    ...
    /daq/acq_c/acq_c -n ${NCHAN} -F ${RATE} -o ${OUTDIR}/${FNAME} -d ${RTD_DT} -m /tmp/rtd/latest_acquisition.data
else echo -e ${ERRBUFF}
fi
